...
class bpro1_process : public vbl_ref_count {
  public:
    ...
    vcl_vector<vcl_string> input_names() const { return input_names_; }

    //: Return the vector of names of the storage classes to be used as output
    vcl_vector<vcl_string> output_names() const { return output_names_; }
    ...
    //: Returns a vector of strings describing the required storage class input
    //: types
    virtual vcl_vector<vcl_string> get_input_type() = 0;
    //: Returns a vector of strings describing the storage class output types
    //: produced
