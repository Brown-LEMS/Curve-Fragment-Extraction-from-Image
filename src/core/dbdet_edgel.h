...
//  Amir Tamrakar 11/15/06          Removed the link graph structures from this
//  class to make it lighter
//
//  Ozge Can Ozcanli Jan 12, 2007   Added copy constructor
//  Ricardo Fabbri Aug 03, 2009     Fixed copy constructor to clone appearance
//  members,
...
typedef vcl_deque<dbdet_edgel*>::const_reverse_iterator
    dbdet_edgel_list_const_reverse_iter;
...
//: edgel class: contains pt, tangent and collection of all the groupings around
//: it
class dbdet_edgel {
  public:
    int id; ///< unique id
    ...
    //: Harris Corner like measure to factor in the error in edge localization
    //: at
    ...
    //: default constructor
    dbdet_edgel()
        : id((unsigned)-1), pt(vgl_point_2d<double>(0, 0)), tangent(0.0),
          strength(0.0), deriv(0.0), uncertainty(0.0),
          left_app(new dbdet_intensity(0.0)),
          right_app(new dbdet_intensity(0.0)) {}

    //: constructor
    //
    dbdet_edgel(vgl_point_2d<double> new_pt, double tan, double conf = 0.0,
                double der = 0.0, double uncer = 0.0,
...
typedef vcl_list<dbdet_edgel_chain*>::const_iterator
    dbdet_edgel_chain_list_const_iter;
