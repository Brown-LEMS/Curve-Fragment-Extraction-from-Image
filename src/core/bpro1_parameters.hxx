...
template <class T> bool bpro1_param_type<T>::set_value(const T& val) {
...
template <class T>
vcl_string bpro1_param_type<T>::create_string(const T& val) const {
...
template <class T>
T bpro1_param_type<T>::parse_string(const vcl_string& input) const {
    T val;
...
#define VIDPRO_PARAMETERS_INSTANTIATE(T) template class bpro1_param_type<T>;
