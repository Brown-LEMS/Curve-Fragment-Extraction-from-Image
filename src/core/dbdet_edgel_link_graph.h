...
struct dbdet_link {
    ...
    vcl_list<dbdet_curvelet*>
        curvelets; ///< list of curvelets containing this link

    //: constructor
    dbdet_link(dbdet_edgel* e1, dbdet_edgel* e2)
        : pe(e1), ce(e2), vote(0.0), deg_overlap(0), flag(false), curvelets(0) {
    }
        ...
        for (; cv_it != curvelets.end(); cv_it++) {
            ...
            for (; cv_it2 != curvelets.end(); cv_it2++) {
                ...
                for (unsigned k = 0; k < cvlet->edgel_chain.size(); k++)
                    duplicate = duplicate && (cvlet->edgel_chain[k] ==
                                              dup_cvlet->edgel_chain[k]);
...
class dbdet_edgel_link_graph {
  public:
    ...
    vcl_vector<bool>
        linked; ///< flag to signal whether this edgel has been linked already
    ...
    //: constructor
    dbdet_edgel_link_graph(int size = 0)
        : cLinks(size), pLinks(size), linked(size), cLinks2(size),
          pLinks2(size) {}
    ...
    // utility functions
    bool is_an_end(int id) {
        return cLinks[id].size() == 0 || pLinks[id].size() == 0;
    }
    ...
    bool is_a_junct(int id) {
        return cLinks[id].size() > 1 || pLinks[id].size() > 1;
    } // X-junction
    ...
    bool edgel_is_legal_first_edgel(dbdet_edgel* eA) {
        return (!linked[eA->id] && // not yet part of any edgel chain
                ...
                pLinks[eA->id].front()->pe !=
                    cLinks[eA->id].front()->ce // not a loop
        );
    }
    ...
    bool edgel_is_legal(dbdet_edgel* eA) {
        return (!linked[eA->id] && // not yet part of any edgel chain
                pLinks[eA->id].size() <=
                    1 && // one link before it (or endpoint) and
