...
class dbdet_edge {
  public:
    ...
    double
        strength; ///< the strength of the edgel (typically gradient magnitude)
    int id;       ///< unique id

    dbdet_edge(vgl_point_2d<double> new_pt, double tan,
               double edge_strength = 10.0)
        : pt(new_pt), tangent(tan), strength(edge_strength) {}
    ~dbdet_edge() {}
};

typedef vbl_array_2d<vcl_vector<dbdet_edgel*>>::iterator dbdet_edgemap_iter;
typedef vbl_array_2d<vcl_vector<dbdet_edgel*>>::const_iterator
    dbdet_edgemap_const_iter;

//: A bucketing structure to hold the edgel tokens. Currently this is
//: implemented
//  simply as a 2d array of tokens the same size as the image that gave rise to
//  it. For expediting linking from this data structure, I am using the
//  dbdet_edgel class to store the edgel tokens instead of the dbdet_edge class
...
class dbdet_edgemap : public vbl_ref_count {
  public:
    ...
    //: constructor
    dbdet_edgemap(int width, int height) : edgels(0) {
        edge_cells.resize(height, width);
    }

    //: constructor2
    dbdet_edgemap(int width, int height, vcl_vector<dbdet_edgel*>& edgels)
        : edgels(0) {
        edge_cells.resize(height, width);
    ...
    unsigned num_edgels() const {
        return edgels.size();
    } ///< number of edgels in the edgemap

    //: read only access
    const vcl_vector<dbdet_edgel*>& cell(int x, int y) const {
        return edge_cells(y, x);
    }
    ...
    bool AlmostEqual(dbdet_edgemap& map, double angle_thresh,
                ...
                for (unsigned int yy = 0; yy < height(); yy++) {
                    ...
                    if (cellA.size() == cellB.size()) {
                        for (unsigned int i = 0; i < cellA.size(); i++) {
                            ...
                            if (vnl_math::abs(cellA[i]->tangent -
                                              cellB[i]->tangent) >
                                angle_thresh) {
                                return false;
                            }
                            if (vnl_math::abs(cellA[i]->strength -
                                              cellB[i]->strength) >
                                strength_thresh) {
                                return false;
                            }
