...
//: Constructor from a parameter block, gradient magnitudes given as an image
//: and directions given as component image
dbdet_nms::dbdet_nms(const dbdet_nms_params& nsp,
                     const vil_image_view<double>& dir_x,
                     const vil_image_view<double>& dir_y,
                     const vil_image_view<double>& grad_mag)
    : thresh_(nsp.thresh_), parabola_fit_type_(nsp.pfit_type_),
      margin_(nsp.margin_), rel_thresh_(nsp.rel_thresh_),
      use_adaptive_thresh_(nsp.use_adaptive_thresh_), dir_x_(dir_x),
      dir_y_(dir_y), grad_mag_(grad_mag), x_(grad_mag.nj(), grad_mag.ni(), 0.0),
...
void dbdet_nms::apply(bool collect_tokens,
                      vcl_vector<vgl_point_2d<double>>& loc,
                      vcl_vector<double>& orientation, vcl_vector<double>& mag,
                      vcl_vector<double>& d2f) {
    vcl_vector<vgl_point_2d<int>> pix_loc;
...
void dbdet_nms::apply(bool collect_tokens,
    ...
    for (unsigned x = margin_; x < grad_mag_.ni() - margin_; x++) {
        for (unsigned y = margin_; y < grad_mag_.nj() - margin_; y++) {
            ...
            if (vcl_abs(direction.x()) < 10e-6 &&
                vcl_abs(direction.y()) < 10e-6)
                continue;
            ...
            if (face_num == -1) {
#ifndef NDEBUG
                vcl_cout << "Ignoring bad gradient direction: " << direction
                         << vcl_endl << " gx: " << gx << " gy: " << gy
                         << vcl_endl << " x: " << x << " y: " << y
                         << " margin: " << margin_
                         << "grad_mag_.ni: " << grad_mag_.ni()
                         << " grad_mag_.nj: " << grad_mag_.nj() << vcl_endl;
                // TODO: investigate the bad gradient directions when they
                // happen and perhaps make sure it can't be fixed instead of
                // ignored
            ...
            double s = intersection_parameter(direction, face_num);
            assert(s != -1000);
            ...
            // Amir: removed this maximum check because, there can be a maxima
            // between f- and f+ even when f<f- or f<f+

            ////Maximum check
            // if ( (f[1]-f[0])>1e-2 && (f[1]-f[2])>1e-2) //epsilon checks
            // instead of absolute
            //{
            // fit a parabola to the data
            double max_val = f[1]; // default (should be updated by parabola
                                   // fit)
            ...
            double s_star =
                (parabola_fit_type_ == dbdet_nms_params::PFIT_3_POINTS)
                    ? subpixel_s(s_list, f, max_val, grad_val)
                    : subpixel_s(x, y, direction, max_val);
    ...
    // post-process the NMS edgel map to reduce the occurrence of duplicate
    // edgels
    for (unsigned x = margin_; x < grad_mag_.ni() - margin_; x++) {
        for (unsigned y = margin_; y < grad_mag_.nj() - margin_; y++) {
            if (mag_(y, x) == 0.0)
            ...
            // use the orientation of the edgel to determine the closest
            // neighbors that could produce a duplicate edgel
            ...
            for (int ii = -1; ii < 2; ii++) {
                for (int jj = -1; jj < 2; jj++) {
                    ...
                    // if there is an edgel at this location, compute the
                    // distance to the current edgel
                    if (mag_(y + jj, x + ii) > 0.0) {
                        double dx = x_(y + jj, x + ii) - x_(y, x);
                        double dy = y_(y + jj, x + ii) - y_(y, x);

                        if ((dx * dx + dy * dy) <
                            0.1) { // closeness threshold, may be made into a
                                   // parameter if anyone cares
    ...
    if (collect_tokens) {
        for (unsigned x = margin_; x < grad_mag_.ni() - margin_; x++) {
            for (unsigned y = margin_; y < grad_mag_.nj() - margin_; y++) {
                ...
                // also return the pixel location so that they can be used for
                // tracing algorithms
...
double dbdet_nms::intersection_parameter(const vgl_vector_2d<double>& direction,
                                         int face_num) {
...
void dbdet_nms::f_values(int x, int y, const vgl_vector_2d<double>& direction,
                         double s, int face_num, double* f) {
    int corners[4];
    get_relative_corner_coordinates(face_num, corners);

    vgl_vector_2d<double> intersection_point = s * direction;
    vgl_vector_2d<double> corner1(
        corners[0], corners[1]); // have to convert to double for subtraction
    vgl_vector_2d<double> corner2(
        corners[2], corners[3]); // have to convert to double for subtraction
    ...
    corner1.set(-corners[0],
                -corners[1]); // have to convert to double for subtraction
    corner2.set(-corners[2],
                -corners[3]); // have to convert to double for subtraction
...
double dbdet_nms::subpixel_s(double* s, double* f, double& max_f,
                             double& max_d) {
    ...
    //  float
    //  gevd_float_operators::InterpolateParabola(float y_1, float y_0, float
    //  y_2,
    //                                            float&y)
    //  {
    //    float diff1 = y_2 - y_1;      // first derivative
    //    float diff2 = 2 * y_0 - y_1 - y_2; // second derivative
    //    y = y_0 + diff1 * diff1 / (8 * diff2);        // interpolate y as
    //    max/min return diff1 / (2 * diff2);   // interpolate x offset
    //}
}

double dbdet_nms::subpixel_s(int x, int y,
...
void dbdet_nms::find_distance_s_and_f_for_point(
    int x, int y, vgl_homg_line_2d<double> line, double& d, double& s,
    const vgl_vector_2d<double>& direction) {
    vgl_homg_point_2d<double> point(x, y);
    vgl_homg_line_2d<double> perp_line =
        vgl_homg_operators_2d<double>::perp_line_through_point(line, point);
    vgl_homg_point_2d<double> intersection_point_homg =
        vgl_homg_operators_2d<double>::intersection(line, perp_line);
    vgl_point_2d<double> intersection_point(intersection_point_homg);
    vgl_vector_2d<double> d_helper(x - intersection_point.x(),
...
//
void dbdet_nms::clear() { mag_.clear(); }
