...
dbdet_sel_process::dbdet_sel_process() {
    ...
    if (
        ...
        //! parameters()->add( "Form Link Graph Using"   , "-linkgraph_algo" ,
        //! linkgraph_choices, 0) || parameters()->add( "Minimum curvelet size
        //! to link" , "-min_size_to_link", (unsigned) 4 ) ||
...
//: Clone the process
bpro1_process* dbdet_sel_process::clone() const {
    return new dbdet_sel_process(*this);
}

//: Return the name of this process
vcl_string dbdet_sel_process::name() { return "Symbolic Edge Linker"; }

//: Return the number of input frame for this process
int dbdet_sel_process::input_frames() { return 1; }

//: Return the number of output frames for this process
int dbdet_sel_process::output_frames() { return 1; }
...
bool dbdet_sel_process::execute() {
    if (input_data_.size() != 1) {
        vcl_cout
            << "In dbdet_sel_process::execute() - not exactly one input \n";
        return false;
    }
    clear_output();
    ...
    dbdet_curvelet_params cvlet_params(
        dbdet_curve_model::CC, nrad, gap, dt, dx, badap_uncer, token_len, max_k,
        max_gamma, bCentered_grouping, bBidirectional_grouping);

    switch (curve_model_type) {
    case 0: // simple linear_model
        cvlet_params.C_type = dbdet_curve_model::LINEAR;
        edge_linker =
            new dbdet_sel_simple_linear(EM, CM, ELG, CFG, cvlet_params);
        break;
    case 1: // linear_model
        cvlet_params.C_type = dbdet_curve_model::LINEAR;
    ...
    case 4: // CC_model with discrete perturbations
        cvlet_params.C_type = dbdet_curve_model::CC;
        edge_linker =
            new dbdet_sel_CC_perturbed(EM, CM, ELG, CFG, cvlet_params);
        break;
    ...
    case 7: // ES_model with discrete perturbations
        cvlet_params.C_type = dbdet_curve_model::ES;
        edge_linker =
            new dbdet_sel_ES_perturbed(EM, CM, ELG, CFG, cvlet_params);
        break;
    }
    ...
    t.mark();
    vcl_cout << "Time taken to form groups and cunstruct curvelet map: "
             << group_time << " sec" << vcl_endl;
    ...
    if (bFormLinkGraph) {
        ...
        edge_linker
            ->regular_contour_filter(); // Filtering step, getting rid of local
                                        // problems, e.g., tiny triangles
        edge_linker->prune_extreme_short_curve_frags(); //  those isolated frags
                                                        //  with only 2 edges
        // edge_linker->pre_processing(); // fill small gaps,
        // edge_linker->merge_extreme_short_curve_frags(); // remove extremely
        // short curve fragments
        edge_linker->refresh_linked_condition();

        if (bGetfinalcontours) {
            if (bDetJct) { // this is an approach integrated with junction
                           // detection and curve extraction
            ...
            } else { // this is a greedy approach via breadth-first search from
                     // each end point
